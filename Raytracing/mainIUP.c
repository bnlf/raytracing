...
int repaint_cb(Ihandle *self)
{
  ...
  if (yc!=height) {
     ...
     glClear(GL_COLOR_BUFFER_BIT);
     glFlush();
     return IUP_DEFAULT; /* esta callback so'desenha depois que o algoritmo termina a imagem */
  }
    ...
    if (yc<height) {
        ...
        glBegin(GL_POINTS);
   		for( x = 0; x < width; ++x ) {
			...
			/* Anti Aliasing basico */
			for ( i=-0.4; i<0.5 ; i+=0.1 )
			{
				if ( i == 0 )
				{
					i+=0.1;
				}
				ray = camGetRay( camera, x+i, yc+i );
				color = colorScale(0.5, colorAddition(color, rayTrace( scene, eye, ray, 0 )));
			}
...
int load_cb(void) {
  ...
  scene = sceneLoad( filename );
  if( scene == NULL ) return IUP_DEFAULT;

  camera = sceneGetCamera( scene );
